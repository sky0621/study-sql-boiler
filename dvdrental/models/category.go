...
func (categoryL) LoadFilmCategories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCategory interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`film_category`), qm.WhereIn(`film_category.category_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func FindCategory(ctx context.Context, exec boil.ContextExecutor, categoryID int, selectCols ...string) (*Category, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"category\" where \"category_id\"=$1", sel,
	)
...
func CategoryExists(ctx context.Context, exec boil.ContextExecutor, categoryID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"category\" where \"category_id\"=$1 limit 1)"
