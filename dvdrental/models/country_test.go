...
func testCountryToManyCities(t *testing.T) {
	...
	if err = a.L.LoadCities(ctx, tx, false, (*[]*Country)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Cities = nil
	if err = a.L.LoadCities(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Cities); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCountryToManyAddOpCities(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		count, err := a.Cities().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	countryDBTypes = map[string]string{`CountryID`: `integer`, `Country`: `character varying`, `LastUpdate`: `timestamp without time zone`}
	_              = bytes.MinRead
)
