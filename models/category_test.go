...
func testCategoryToManyFilmCategories(t *testing.T) {
	...
	if err = a.L.LoadFilmCategories(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
var (
	categoryDBTypes = map[string]string{`CategoryID`: `integer`, `Name`: `character varying`, `LastUpdate`: `timestamp without time zone`}
	_               = bytes.MinRead
)
