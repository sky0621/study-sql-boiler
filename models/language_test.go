...
func testLanguagesExists(t *testing.T) {
	...
	e, err := LanguageExists(ctx, tx, o.LanguageID)
	if err != nil {
...
func testLanguagesFind(t *testing.T) {
	...
	languageFound, err := FindLanguage(ctx, tx, o.LanguageID)
	if err != nil {
		t.Error(err)
	}
...
func testLanguageToManyFilms(t *testing.T) {
	...
	if err = a.L.LoadFilms(ctx, tx, false, (*[]*Language)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Films = nil
	if err = a.L.LoadFilms(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Films); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testLanguageToManyAddOpFilms(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		count, err := a.Films().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	languageDBTypes = map[string]string{`LanguageID`: `integer`, `Name`: `character`, `LastUpdate`: `timestamp without time zone`}
	_               = bytes.MinRead
)
